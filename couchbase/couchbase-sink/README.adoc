...
> wget https://repo1.maven.org/maven2/org/apache/camel/kafkaconnector/camel-couchbase-kafka-connector/0.10.0/camel-couchbase-kafka-connector-0.10.0-package.tar.gz
> untar.gz camel-couchbase-kafka-connector-0.10.0-package.tar.gz
```
There is a little bug in 0.10.0, so we'll need to add explicitly the couchbase driver
